...
export class UserServices implements IUserServices {
  ...
  async create(name: string, email: string, password: string, avatar?: string): Promise<IUser> {
  ...
  async changePassword(id: string, currentPassword: string, newPassword: string): Promise<object> {
    if ((await this.idAlreadyExists(id)) === false) {
      throw new Error('User not found')
    }
